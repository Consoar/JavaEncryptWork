...
public class SMS4 {
	...
	void SMS4Crypt(byte[] Input, int[] rk) {
		...
		for (int j = 0; j < 16; j += 4) {
			Input[j] = (byte) (x[3 - j / 4] >>> 24 & 0xFF);
			Input[j + 1] = (byte) (x[3 - j / 4] >>> 16 & 0xFF);
			Input[j + 2] = (byte) (x[3 - j / 4] >>> 8 & 0xFF);
			Input[j + 3] = (byte) (x[3 - j / 4] & 0xFF);
		}
	}
	...
	public byte[] encrypt(byte[] data) {
		...
		while (len >= BLOCK) {
			...
			SMS4Crypt(input, round_key);
			System.arraycopy(input, 0, output, point, BLOCK);
			len -= BLOCK;
			point += BLOCK;
		}
		return output;
